        ...
        {payments.length > 0 ? (
          payments.map((payment) => (
            <div key={payment._id} className="order-card">
              <h3>{payment.orderId?._id}</h3>
              <p><strong>Name:</strong> {payment.orderId?.name}</p>
              <p><strong>Email:</strong> {payment.orderId?.email}</p>
              <p><strong>Payment Method:</strong> {payment.paymentMethod}</p>
              <p><strong>Amount:</strong> ₹{payment.amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</p>
