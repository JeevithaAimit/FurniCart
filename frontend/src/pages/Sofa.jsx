...
const Sofa = () => {
  ...
  const sofaTypes = [
    "All", "1Seater Sofa", "2Seater (F/W)", "3Seater (F/W)",
    "3+2+1Sofa Set", "3+2Sofa Set", "3+1+1Sofa Sets (F/W)"
  ];
  ...
  const priceRanges = [
    ...
    { label: "80k-1 Lakh", min: 80000, max: 100000 },
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log("Sofa Products Types:", sofaProducts.map(p => p.type)); // <== Add this
  ...
  const applyFilters = () => {
    ...
    console.log("All Sofa Types in DB:", products.map(p => p.type)); // 🔍 See raw data
    console.log("Selected Type:", selectedType); // 🔍 What you're trying to filter by
    ...
    console.log("Filtered by Type:", filtered.length); // 🔍 How many remain
          ...
          <div className="product-grid">
            {currentProducts.length > 0 ? (
             currentProducts.map((product) => {
              console.log("Product:", product); // ✅ OK inside curly braces
